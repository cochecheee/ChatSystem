    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 600, fontSize: 13.5 }}>Tổng quan rủi ro dự án</div>
      ...
      <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {data.top_risks.length > 0 && (
          <Section
            icon="🚨"
            title="Rủi ro hàng đầu"
            badge={`${data.top_risks.length}`}
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {data.top_risks.map((r) => (
                <RiskRow
                  key={r.finding_id}
                  risk={r}
                  onClick={() => onOpenFinding?.(r.finding_id)}
                />
              ))}
            </div>
          </Section>
        ...
        <Section icon="💡" title="Khuyến nghị">
          <MiniMarkdown text={data.recommendations_md} />
        </Section>

        <Section icon="📈" title="Tình trạng pipeline" inline>
          <span>
      ...
      >
        Tạo bởi <code className="mono">{data.model}</code> · cập nhật mỗi ~10 phút
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          <span
            style={{ color: SEV_COLOR[risk.severity], fontWeight: 600, textTransform: 'uppercase' }}
          >
            {risk.severity}
          </span>
          <code
            className="mono"
            style={{ background: 'var(--bg-1)', padding: '0 4px', borderRadius: 3 }}
          >
            {risk.rule_id}
          </code>
          <span
      ...
      <Icon
        name="chevron_right"
        size={11}
        style={{ color: 'var(--fg-3)', flexShrink: 0, marginTop: 4 }}
      />
    </button>
  );
}

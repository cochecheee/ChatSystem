...
export function AreaTrend({ values, values2, height = 200 }: AreaTrendProps) {
  const w = 720, h = height, padX = 8, padT = 12, padB = 28;
  const max = Math.max(...values, ...(values2 ?? [0]));
  const stepX = (w - padX * 2) / (values.length - 1);
  const yFor = (v: number) => padT + (1 - v / max) * (h - padT - padB);
  const toPath = (arr: number[]) => arr.map((v, i) => `${i === 0 ? 'M' : 'L'} ${padX + i * stepX} ${yFor(v)}`).join(' ');
  const toArea = (arr: number[]) => `${toPath(arr)} L ${padX + (arr.length - 1) * stepX} ${h - padB} L ${padX} ${h - padB} Z`;
  const ticks = 4;
  const gridY = Array.from({ length: ticks }, (_, i) => padT + (i / (ticks - 1)) * (h - padT - padB));
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} width="100%" height={h} preserveAspectRatio="none" style={{ display: 'block' }}>
      ...
      {gridY.map((y, i) => <line key={i} x1={padX} x2={w - padX} y1={y} y2={y} stroke="var(--line)" strokeDasharray="2 4" />)}
      {values2 && <path d={toPath(values2)} stroke="var(--fg-4)" strokeWidth="1.5" fill="none" strokeDasharray="3 3" />}
      ...
      {xLabels.map((lbl, i) => {
        ...
        return <text key={i} x={x} y={h - 8} fontSize="10" fill="var(--fg-4)" textAnchor="middle" fontFamily="var(--font-mono)">{lbl}</text>;
      })}
    </svg>
  );
}
...
export function SeverityBar({ counts, height = 6 }: { counts: Record<string, number>; height?: number }) {
  const total = (counts.critical || 0) + (counts.high || 0) + (counts.medium || 0) + (counts.low || 0);
  if (!total) return <div className="sev-bar" style={{ height }}><div style={{ background: 'var(--line-strong)', width: '100%' }} /></div>;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {total > 0 && data.map((d, i) => {
        ...
        const offset = -acc;
        acc += len;
        return <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={d.c} strokeWidth="14" strokeDasharray={dash} strokeDashoffset={offset} transform={`rotate(-90 ${cx} ${cy})`} />;
      })}
      <text x={cx} y={cy} textAnchor="middle" dominantBaseline="middle" fontSize="22" fontWeight="600" fill="var(--fg)" fontFamily="var(--font-sans)">{total}</text>
      <text x={cx} y={cy + 16} textAnchor="middle" fontSize="10" fill="var(--fg-3)" fontFamily="var(--font-sans)">issues</text>
    </svg>
  );
}

export function Heatmap({ rows = 4, cols = 24 }: { rows?: number; cols?: number }) {
  const cells = Array.from({ length: rows * cols }, (_, i) => Math.abs((Math.sin(i * 12.9898) * 43758.5453) % 1));
    ...
    <svg width="100%" viewBox={`0 0 ${w} ${h + 18}`} preserveAspectRatio="none">
      {Array.from({ length: rows }).map((_, r) =>
        Array.from({ length: cols }).map((_, c) => {
          ...
          return <rect key={`${r}-${c}`} x={c * (cellW + gap)} y={r * (cellH + gap)} width={cellW} height={cellH} rx="2" fill="var(--accent)" opacity={0.08 + v * 0.85} />;
        })
      )}
      <text x="0" y={h + 14} fontSize="9.5" fill="var(--fg-4)" fontFamily="var(--font-mono)">00:00</text>
      <text x={w / 2 - 12} y={h + 14} fontSize="9.5" fill="var(--fg-4)" fontFamily="var(--font-mono)">12:00</text>
      <text x={w - 30} y={h + 14} fontSize="9.5" fill="var(--fg-4)" fontFamily="var(--font-mono)">23:00</text>
    </svg>
  );
}
